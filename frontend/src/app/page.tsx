  ...
  return (
    <div className="min-h-screen bg-black text-white selection:bg-yellow-500/30 relative">
      ...
      <section id="herramientas" className="max-w-7xl mx-auto px-6 py-32">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 auto-rows-[300px]">
          <ToolCard 
            ...
            desc="Sube tus libros y hazles preguntas. Potenciado por Gemini 1.5 Flash."
            tag="NotebookLM • IA v1.5"
          />
          <ToolCard 
            href="/convert"
            className="md:col-span-2"
            icon={<FileSpreadsheet className="w-16 h-16 text-white opacity-80" />}
            title="Conversor Total"
            desc="Cambia de PDF a Word, Excel o PPT en segundos manteniendo el formato original."
            tag="Local"
          />
          <ToolCard 
            href="/transcribe"
            className="md:col-span-1"
            icon={<AudioLines className="w-16 h-16 text-yellow-400 opacity-90" />}
            title="Audio-Notas"
            desc="Transcripción y resúmenes de tus clases grabadas."
            tag="Whisper-3"
          />
          <ToolCard 
            href="/chat"
            className="md:col-span-1"
            icon={<BrainCircuit className="w-16 h-16 text-white opacity-80" />}
            title="Sintetizador"
            desc="Resúmenes de libros de 500+ páginas al instante usando Gemini 1.5 Flash."
      ...
      <section className="max-w-5xl mx-auto px-6 py-40 text-center">
        <div className="p-16 rounded-[4rem] bg-yellow-400 text-black relative overflow-hidden">
          <div className="relative z-10">
            <h2 className="text-5xl font-black tracking-tighter mb-6">¿ERES PROGRAMADOR?</h2>
            <p className="text-black/70 text-xl mb-10 max-w-2xl mx-auto font-bold">Únete al equipo de desarrollo. Si estudias Informática o Sistemas, contribuye en GitHub y mejora la IA del campus.</p>
            <button className="px-10 py-4 rounded-2xl bg-black text-white font-black flex items-center gap-3 mx-auto hover:bg-gray-900 transition-all">
              <Code2 className="w-6 h-6" /> Ver Repositorio
            </button>
          </div>
        </div>
      </section>
...
function ToolCard({ icon, title, desc, className, href, tag }: { icon: React.ReactNode, title: string, desc: string, className?: string, href: string, tag: string }) {
  return (
    <motion.div whileHover={{ y: -5 }} className={`group relative p-10 rounded-[3rem] glass-card-aauca flex flex-col justify-between overflow-hidden ${className}`}>
      <Link href={href} className="absolute inset-0 z-20" />
      <div className="flex justify-between items-start">
        <div className="p-4 rounded-2xl bg-white/5 group-hover:bg-yellow-400/10 transition-colors">{icon}</div>
        <span className="text-[9px] font-black uppercase tracking-widest text-gray-500">{tag}</span>
      </div>
      <div>
        <h3 className="text-2xl font-black mb-3 group-hover:text-yellow-400 transition-colors">{title}</h3>
        <p className="text-gray-500 text-sm leading-relaxed font-medium group-hover:text-gray-400 transition-colors">{desc}</p>
      </div>
    </motion.div>
  );
}
